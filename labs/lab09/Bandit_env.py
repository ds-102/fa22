...
class BanditEnv():
	...
	def initialize(self,make_plot=1,alg_defined=0,alg=None):
		...
		if make_plot:
			...
			self.figure.canvas.manager.set_window_title('Bandits Demo')
			self._plot_dists()
	...
	def _plot_dists(self):
		...
		for ax in self.axes:
			for arm in range(self.num_arms):
				...
				a=sns.kdeplot(y=np.random.normal(mu,sigma**2,(100000)), color=self.COLOURS[arm],shade=False,alpha=1.0,ax=ax,lw=1.2,ls='-')
